...
- OpenCode defines three subagents (`analyze`, `placement`, `critic`); each writes exactly one requested artifact. Only the orchestrator owns ordering, retries, render views, stop conditions, and human gates.
...
- `initialize_placement.py` writes both `placement_init.json` and `iterations/001/assembly.json`. The placement subagent applies critic deltas only, skipping `locked` links and using the best prior layout after regressions.
