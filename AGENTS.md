...
- One-time placement inputs: `analyze` -> `parts.json` -> **human parts gate** -> `generate_components.py` -> `component_images/` + `component_glbs/` + `component_dims.json` -> `initialize_placement.py` -> `iterations/001/assembly.json`. Each part needs a specific `name` matching its factual `description`; descriptions are copied into image prompts.
- Placement loop under `iterations/<NNN>/`: `initialize_placement.py` writes `iterations/001/assembly.json`; later `assembly.json` files are written by `tool_scripts/update_placement.py` applying critic corrections and recomputing Blender transforms. Each iteration then runs `blender_assemble.py` -> `assembled.blend` -> orchestrator-written `render_views.json` -> `blender_render_views.py` -> `renders/` -> `critic` -> `critic.json`.
...
- `parts.json` stores per-part `world_size`, `world_center`, and `euler_deg` in metres for every part including the root. `initialize_placement.py` and `update_placement.py` derive `node_scale` and `node_origin` from `component_dims.json` and write them into `assembly.json`. `blender_assemble.py` applies those pre-computed transforms directly.
- `update_placement.py` writes subsequent `assembly.json` files by assigning corrected world values from the critic, skipping `locked` parts, then recomputing `node_scale` / `node_origin` for all parts. Accepts `--prev-assembly` as the best prior layout after regressions.
- Critic `locked` means scale, position, and orientation are all correct; feedback is per-component via `corrected_world_size`, `corrected_world_center`, `suggested_rotation_delta`, or `locked`.
...
- Placement init is `tool_scripts/initialize_placement.py --run-dir <run_dir>`; paths come from `configs/base.yaml` `placement_init` block.
