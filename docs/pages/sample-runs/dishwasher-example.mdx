...
## 5. initialize_placement.py

This script reads poses from `parts.json` and raw mesh sizes from `component_dims.json`. It writes `placement_init.json` and `iterations/001/assembly.json`. No subagent is involved.
...
### 6a. blender_assemble.py

Loads each GLB, parents parts in the kinematic tree, and positions meshes to match `world_size`, `world_center`, and `euler_deg` in `assembly.json`.
...
### 6e. update_placement.py (iteration 2 onward)
