...
Dexter has no standalone Python driver. The **orchestrator** OpenCode agent sequences two subagents and deterministic tool scripts, pausing at two human gates.
...
│   │   ├── world_dims.json            # per-part world dimensions
...
|----------|-------------------|-------------|
| `source.png` | Run start (copy) | analyze, generate_components |
| `parts.json` | Step 1 | generate_components, initialize_placement |
| `component_images/` | Step 2 | generate_components (fal step) |
| `component_glbs/` | Step 2 | blender_assemble |
| `component_dims.json` | Step 2 | initialize_placement, compute_world_dims |
| `placement_init.json` | Step 3 | iteration 001 assembly |
| `iterations/N/assembly.json` | Step 3–4 | blender_assemble, compute_world_dims |
| `iterations/N/world_dims.json` | Step 4 | critic |
| `iterations/N/assembled.blend` | Step 4 | blender_render_views, USD export |
| `iterations/N/renders/` | Step 4 | critic |
...
After you approve the parts, `generate_components.py` runs OpenAI once per part (using `source.png` as reference), fal.ai for GLBs, and Blender to measure each mesh.

**Who:** orchestrator → `generate_components.py`  
...
The script skips parts whose PNG, GLB, or dims already exist, so re-running only fills in missing assets.

---

### Step 3 — Placement init (tool script)

`initialize_placement.py` combines geometry from `parts.json` with measured mesh sizes to write `placement_init.json` and `iterations/001/assembly.json`.

**Who:** orchestrator → `initialize_placement.py`  
**Outputs:**

| File | Description |
|------|-------------|
| `placement_init.json` | `root_scale`, per-part `child_scale`, closed/open poses |
| `iterations/001/assembly.json` | First layout IR |
...
Blender assembles and renders four views. `compute_world_dims.py` pre-computes per-part world dimensions. The critic scores the result and suggests fixes. `apply_critic.py` applies those fixes deterministically on the next iteration. The orchestrator keeps the best-scoring iteration.

**Who:** `critic` subagent; `apply_critic.py`, `blender_assemble.py`, `blender_render_views.py`, `compute_world_dims.py`  
...
After five rounds of critique and `apply_critic.py` refinements, iteration 6 was the best layout. The upper-rack correction from iteration 5 was applied — the rack sits further back in the cavity, matching the source photo more closely. Cabinet, door, and lower rack stayed locked.
...
**Iteration 2+ behavior:** `apply_critic.py` reads the previous best `assembly.json` + `critic.json` and applies only the critic's corrections. Components marked `locked: true` are not changed.
