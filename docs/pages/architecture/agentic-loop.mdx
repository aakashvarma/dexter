...
`initialize_placement.py` reads world-space poses from `parts.json` and raw mesh sizes from `component_dims.json` to write `placement_init.json` and `iterations/001/assembly.json`. Fully deterministic — no LLM.

### 4 — Render, critique, refine
...
If the loop continues, `update_placement.py` applies critic corrections to the next `assembly.json`. `locked` parts are unchanged. After a regression, the orchestrator can base the next iteration on the best-scoring layout.
...
| **Placement init** | `placement_init.json` | initialize_placement | iteration 001 assembly |
| **Component dims** | `component_dims.json` | generate_components → blender_measure_glbs | initialize_placement, blender_assemble |
...
- `world_size` (root) → real-world dimensions in metres
- Non-root `world_center` / `euler_deg` → copied into the first `assembly.json`
...
`blender_assemble.py` uses `component_dims.json` internally so each GLB's world bounding box matches the stored values — you work in world-space metres, not Blender-local transforms.
...
**`placement_init.json`** — pre-computed scales and Blender node offsets. If init looks wrong, fix `parts.json` and delete `placement_init.json` plus `iterations/001/assembly.json` to regenerate.
...
source.png
  ...
  → placement_init.json + iterations/001/assembly.json
...
**`initialize_placement.py`** — Combines `parts.json` poses with raw mesh sizes to compute Blender `node_scale` and `node_origin`. Writes both `placement_init.json` and `iterations/001/assembly.json`.
...
<Callout type="info" emoji="🔄">
**`update_placement.py`** — Copies the previous layout and applies critic corrections. After a regression, pass the best-scoring layout as `--prev-assembly`.
...
Prints `OK` on success; prints each error and exits 1 on failure. When validation fails, the orchestrator re-invokes the same subagent up to `loop.max_validation_retries` (default: 2).
...
| `placement_init.schema.json` | `placement_init.json` | initialize_placement |
