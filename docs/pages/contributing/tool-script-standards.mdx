...
- Validate JSON inputs against `schemas/` before processing; validate outputs before finishing. On `KeyError` / `ValueError` from placement math, exit with `Error: …` so agents can fix the source file.
...
- Validation: `validate_schema(name, path)` at script entry (calls `exit_if_invalid_json` with fix hints); CLI: `python3 tool_scripts/common.py --schema … --data …` (fixtures in `test_fixtures/`)
- Placement math: `recompute_blender_transforms()` in `common.py` (used by `initialize_placement.py` and `update_placement.py`)
